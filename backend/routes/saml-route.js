...
const idp = new saml.IdentityProvider({
  ...
  privateKey: fs.readFileSync(path.join(__dirname, '../certs/encryptKey.pem')),
  ...
  messageSigningOrder: saml.Constants.MessageSignatureOrder.ETS,
  ...
  getOktaUser(nameId).then((user) => {
    createOnfidoApplicant(user.profile.firstName, user.profile.lastName, user.profile.email).then((applicant) => {
      updateOktaUser(nameId, applicant.id, '').then((updated) => {
        return res.redirect(`${process.env.FRONTEND_URL}/idv?applicant=${applicant.id}&RelayState=${relayState}`);
      }).catch((error) => {
        return res.status(500).json({ error });
      });
    }).catch((error) => {
...
router.get('/response', async (req, res) => {
  ...
  getOktaUserByApplicant(applicant).then( async (user) => {
    try {
      ...
      console.log('post saml build');
      console.log(context);
      return res.status(200).json({ samlMessage: context, acs: acs });
    } catch (error) {
      return res.status(500).json({ error });
    }
  }).catch((error) => {
